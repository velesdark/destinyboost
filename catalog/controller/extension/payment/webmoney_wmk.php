...
class ControllerExtensionPaymentWebmoneyWMK extends Controller
{
    ...
    public function index()
    {
        ...
        $data['params'] = array(
            'LMI_PAYEE_PURSE' => $this->config->get('webmoney_wmk_merch_r'),  // Номер кошелька
            'LMI_PAYMENT_AMOUNT' => $amount,
            'LMI_PAYMENT_NO' => $this->session->data['order_id']
        );

        $LMI_PAYMENTFORM_SIGN = implode(';', array_values($data['params'])) . ';' . $this->config->get('webmoney_wmk_secret_key_x20') . ';';
        ...
        return $this->load->view('extension/payment/webmoney_wmk.tpl', $data);
    }
    ...
    public function confirm()
    {
        if (!empty($this->session->data['order_id']) && ($this->session->data['payment_method']['code'] == 'webmoney_wmk')) {
            $this->load->model('checkout/order');
            $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('webmoney_wmk_order_confirm_status_id'));
        }
    }
    ...
    public function fail()
    {
        ...
        if ($LMI_PAYMENT_NO) {
            $this->load->model('checkout/order');
            $this->model_checkout_order->addOrderHistory($LMI_PAYMENT_NO, $this->config->get('webmoney_wmk_order_fail_status_id'), 'Webmoney WMK Fail', true);
        }
    ...
    public function callback()
    {
        ...
        $LMI_SECRET_KEY = $this->config->get('webmoney_wmk_secret_key');
        ...
        if ($amount_order != $amount_payment) {
            ...
            $this->model_checkout_order->addOrderHistory($LMI_PAYMENT_NO, $this->config->get('webmoney_wmk_order_fail_status_id'), 'Webmoney WMK Fraud: ' . $LMI_PAYER_PURSE, true);
            return 0;
        }

        if ($LMI_PAYEE_PURSE != $this->config->get('webmoney_wmk_merch_r')) {
            $this->sendForbidden(sprintf($this->language->get('error_merch_r'), $LMI_PAYEE_PURSE));
            return 0;
        }

        if ($order_info['order_status_id'] != $this->config->get('webmoney_wmk_order_status_id')) {
            $this->model_checkout_order->addOrderHistory($LMI_PAYMENT_NO, $this->config->get('webmoney_wmk_order_status_id'), 'Webmoney WMK Success: ' . $LMI_PAYER_PURSE, true);
        }
    ...
    {
        switch ($this->config->get('webmoney_wmk_log')) {
            case self::$LOG_OFF:
                return;
            case self::$LOG_SHORT:
        ...
        if (!$this->log) {
            $this->log = new Log($this->config->get('webmoney_wmk_log_filename'));
        }
