...
class ControllerExtensionPaymentWebmoneyWME extends Controller
{
    ...
    public function index()
    {
        ...
        $data['params'] = array(
            'LMI_PAYEE_PURSE' => $this->config->get('webmoney_wme_merch_r'),  // Номер кошелька
            'LMI_PAYMENT_AMOUNT' => $amount,
            'LMI_PAYMENT_NO' => $this->session->data['order_id']
        );

        $LMI_PAYMENTFORM_SIGN = implode(';', array_values($data['params'])) . ';' . $this->config->get('webmoney_wme_secret_key_x20') . ';';
        ...
        return $this->load->view('extension/payment/webmoney_wme.tpl', $data);
    }
    ...
    public function confirm()
    {
        if (!empty($this->session->data['order_id']) && ($this->session->data['payment_method']['code'] == 'webmoney_wme')) {
            $this->load->model('checkout/order');
            $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('webmoney_wme_order_confirm_status_id'));
        }
    }
    ...
    public function fail()
    {
        ...
        if ($LMI_PAYMENT_NO) {
            $this->load->model('checkout/order');
            $this->model_checkout_order->addOrderHistory($LMI_PAYMENT_NO, $this->config->get('webmoney_wme_order_fail_status_id'), 'Webmoney WME Fail', true);
        }
    ...
    public function callback()
    {
        ...
        if ($amount_order != $amount_payment) {
            ...
            $this->model_checkout_order->addOrderHistory($LMI_PAYMENT_NO, $this->config->get('webmoney_wme_order_fail_status_id'), 'Webmoney WME Fraud: ' . $LMI_PAYER_PURSE, true);
            return 0;
        }

        if ($LMI_PAYEE_PURSE != $this->config->get('webmoney_wme_merch_r')) {
        ...
        if ($order_info['order_status_id'] != $this->config->get('webmoney_wme_order_status_id')) {
            $this->model_checkout_order->addOrderHistory($LMI_PAYMENT_NO, $this->config->get('webmoney_wme_order_status_id'), 'Webmoney WME Success: ' . $LMI_PAYER_PURSE, true);
        }
    ...
    {
        switch ($this->config->get('webmoney_wme_log')) {
            case self::$LOG_OFF:
                return;
            case self::$LOG_SHORT:
        ...
        if (!$this->log) {
            $this->log = new Log($this->config->get('webmoney_wme_log_filename'));
        }
