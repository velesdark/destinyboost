...
class ControllerExtensionModuleAmazonPay extends Controller
{
    public function index()
    {
        ...
        if ($this->config->get('amazon_login_pay_status') && $this->config->get('amazon_pay_status') && (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) && !($this->config->get('amazon_login_pay_minimum_total') > 0 && $this->config->get('amazon_login_pay_minimum_total') > $this->cart->getSubTotal())) {
