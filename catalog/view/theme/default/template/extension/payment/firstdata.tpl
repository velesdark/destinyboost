<form action="<?php echo $action; ?>" method="POST" class="form-horizontal" id="firstdata_form_redirect">
    <fieldset id="payment">
        ...
        <p><input type="radio" name="hosteddataid"
                  value="<?php echo $card['token']; ?>" <?php echo ($i == 1 ? ' checked="checked"' : ''); ?>
            /> <?php echo $card['digits'] . ' - ' . $card['expire_month'] . '/' . $card['expire_year']; ?></p>
        <?php $i++; ?>
        <?php } ?>
        <p><input type="radio" name="hosteddataid"
                  value="<?php echo $new_hosted_id; ?>" <?php echo $i == 1 ? ' checked="checked"' : ''; ?> /> Use a new
            card</p>
        <?php } else { ?>
