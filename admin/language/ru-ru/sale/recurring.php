<?php
// Heading
$_['heading_title'] = 'Периодические платежи';

// Text
$_['text_success'] = 'Список периодических платежей успешно обновлен';
$_['text_list'] = 'Список профилей';
$_['text_recurring_detail'] = 'Информация об отложенном платеже';
$_['text_order_detail'] = 'Информация о заказе';
$_['text_product_detail'] = 'Информация о товаре';
$_['text_transaction'] = 'Транзакции';
$_['text_order_recurring_id'] = 'Отложенный платеж ID';
$_['text_reference'] = 'Оплата';
$_['text_recurring_name'] = 'Профиль возврата';
$_['text_recurring_description'] = 'Описание';
$_['text_recurring_status'] = 'Статус возврата';
$_['text_payment_method'] = 'Метод платежа';
$_['text_order_id'] = 'Заказ ID';
$_['text_customer'] = 'Покупатель';
$_['text_email'] = 'Email';
$_['text_date_added'] = 'Дата добавления';
$_['text_order_status'] = 'Статус заказа';
$_['text_type'] = 'Тип';
$_['text_action'] = 'Действие';
$_['text_product'] = 'Товар';
$_['text_quantity'] = 'Колл.';
$_['text_amount'] = 'Сумма';
$_['text_cancel_payment'] = 'Отмена платежа';
$_['text_status_1'] = 'Активирован';
$_['text_status_2'] = 'Деактивирован';
$_['text_status_3'] = 'Отменен';
$_['text_status_4'] = 'Приостановлен';
$_['text_status_5'] = 'Истекший';
$_['text_status_6'] = 'В ожидании';

$_['text_transactions'] = 'Операции';
$_['text_cancel_confirm'] = 'Это действие невозможно отменить! Вы уверены?';
$_['text_transaction_date_added'] = 'Дата добавления';
$_['text_transaction_payment'] = 'Платеж';
$_['text_transaction_outstanding_payment'] = 'Платеж просрочен';
$_['text_transaction_skipped'] = 'Платеж пропущен';
$_['text_transaction_failed'] = 'Платеж не прошел';
$_['text_transaction_cancelled'] = 'Отменённые';
$_['text_transaction_suspended'] = 'Приостановлен';
$_['text_transaction_suspended_failed'] = 'Приостановлен - оплата не прошла';
$_['text_transaction_outstanding_failed'] = 'Неоплачен - ошибка платежа';
$_['text_transaction_expired'] = 'Истекший';
$_['text_cancelled'] = 'Периодических платеж был успешно отменен';

// Column
$_['column_order_recurring_id'] = 'Отложенный ID';
$_['column_order_id'] = 'Заказ ID';
$_['column_reference'] = 'Ссылка';
$_['column_customer'] = 'Покупатель';
$_['column_date_added'] = 'Дата добавления';
$_['column_status'] = 'Статсу';
$_['column_amount'] = 'Сумма';
$_['column_type'] = 'Тип';
$_['column_action'] = 'Действие';

// Entry
$_['entry_order_recurring_id'] = 'ID';
$_['entry_order_id'] = 'Заказ №';
$_['entry_reference'] = 'Ссылка на платеж';
$_['entry_customer'] = 'Покупатель';
$_['entry_date_added'] = 'Дата добавления';
$_['entry_status'] = 'Статус';
$_['entry_type'] = 'Тип';
$_['entry_action'] = 'Действие';
$_['entry_email'] = 'Email';
$_['entry_description'] = 'Описание периодических платежей';
$_['entry_product'] = 'Товар';
$_['entry_quantity'] = 'Кол-во';
$_['entry_amount'] = 'Сумма';
$_['entry_recurring'] = 'Профиль платежа';
$_['entry_payment_method'] = 'Метод оплаты';
$_['entry_cancel_payment'] = 'Отмененный платеж';

// Error
$_['error_not_cancelled'] = 'Ошибка: %s';
$_['error_not_found'] = 'Периодических платеж отменить не удалось';
