    ...
    <table class="form simple-custom-form">
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="<?php echo $field['id'] ?>" value="<?php echo $field['value'] ?>"
                       class="form-control <?php echo $field['type'] == 'date' ? 'datepicker' : '' ?> <?php echo $field['type'] == 'time' ? 'timepicker' : '' ?>">
                <?php } ?>
                ...
                <select name="<?php echo $field['id'] ?>">
                    ...
                    <option value="<?php echo trim($key) ?>"
                    <?php echo trim($key) == $field['value'] ? ' selected="selected"' : ''?>
                    ><?php echo $value ?></option>
                    <?php } ?>
                </select>
                <?php } else { ?>
                <div class="simple-values" data-name="<?php echo $field['id'] ?>"
                     data-source="<?php echo $field['values'] ?>" data-type="select"
                     data-value="<?php echo $field['value'] ?>"></div>
                <?php } ?>
                <?php } ?>
                <?php if ($field['type'] == 'radio' || $field['type'] == 'radio_from_api') { ?>
                <?php if (is_array($field['values'])) { ?>
                <?php foreach ($field['values'] as $key => $value) { ?>
                <label><input type="radio" name="<?php echo $field['id'] ?>"
                              value="<?php echo trim($key) ?>" <?php echo trim($key) == $field['value'] ? ' checked="checked"' : ''?>
                    >&nbsp;<?php echo $value ?></label><br>
                <?php } ?>
                <?php } else { ?>
                <div class="simple-values" data-name="<?php echo $field['id'] ?>"
                     data-source="<?php echo $field['values'] ?>" data-type="radio"
                     data-value="<?php echo $field['value'] ?>"></div>
                <?php } ?>
                <?php } ?>
                ...
                <label><input type="checkbox" name="<?php echo $field['id'] ?>[]"
                              value="<?php echo trim($key) ?>" <?php echo is_array($field['value']) && in_array(trim($key), $field['value']) ? ' checked="checked"' : ''?>
                    >&nbsp;<?php echo $value ?></label><br>
                <?php } ?>
                <?php } else { ?>
                <div class="simple-values" data-name="<?php echo $field['id'] ?>"
                     data-source="<?php echo $field['values'] ?>" data-type="checkbox"
                     data-value="<?php echo implode(',', $field['value']) ?>"></div>
                <?php } ?>
                <?php } ?>
        ...
        <tr>
            ...
            <td><a class="button btn btn-primary"
                   onclick="submit_<?php echo $form_id ?>();return false;"><?php echo $button_save ?></a></td>
        </tr>
        <?php } ?>
    </table>
</form>
