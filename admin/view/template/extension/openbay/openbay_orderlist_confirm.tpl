...
<div id="content">
    <div class="page-header">
        <div class="container-fluid">
            <div class="pull-right"><a href="<?php echo $cancel; ?>" data-toggle="tooltip"
    ...
    <div class="container-fluid">
        <div class="alert alert-warning"><?php echo $text_confirm_change_text; ?>:
            <strong><?php echo $status_mapped[$change_order_status_id]; ?></strong></div>
        <form action="<?php echo $link_complete; ?>" method="post" enctype="multipart/form-data" id="form" class="form">
            <table class="table table-bordered table-hover">
                ...
                <input type="hidden" name="old_status[<?php echo $order['order_id']; ?>]"
                       value="<?php echo $order['order_status_id']; ?>"/>
                <tr>
                    <td class="text-center"><?php echo $order['order_id']; ?></td>
                    <td class="text-left"><?php echo $order['customer']; ?></td>
                    <td class="text-left"><?php echo $order['status']; ?></td>
                    <td class="text-left">
                        <input type="hidden" name="channel[<?php echo $order['order_id']; ?>]"
                               value="<?php echo $order['channel']; ?>"/>
                    ...
                    <td class="text-left">
                        ...
                        <div class="form-group">
                            ...
                            <select name="carrier[<?php echo $order['order_id']; ?>]" class="form-control">
                                ...
                                <option
                                <?php echo ($carrier['description'] == $order['shipping_method'] ? ' selected' : ''); ?>
                                ><?php echo $carrier['description']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" name="tracking[<?php echo $order['order_id']; ?>]" value=""
                                   placeholder="<?php echo $entry_tracking_no; ?>" class="form-control"/>
                        </div>
                        <?php } ?>
                        <?php } ?>
                        ...
                        <div class="form-group">
                            ...
                            <select name="carrier[<?php echo $order['order_id']; ?>]"
                                    class="form-control amazon_carrier"
                                    id="amazon_carrier_<?php echo $order['order_id']; ?>">
                                <?php foreach($market_options['amazon']['carriers'] as $courier) { ?>
                                <?php echo '<option'.($courier == $market_options['amazon']['default_carrier'] ? ' selected' : '').'>
                                '.$courier.'</option>'; ?>
                                <?php } ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="carrier_other[<?php echo $order['order_id']; ?>]" value=""
                                   placeholder="<?php echo $entry_other; ?>" class="form-control"
                                   id="amazon_carrier_<?php echo $order['order_id']; ?>_other"/>
                        </div>
                        <div class="form-group">
                            <label class="control-label"><?php echo $entry_tracking_no; ?></label>
                            <input type="text" name="tracking[<?php echo $order['order_id']; ?>]" value=""
                        ...
                        <div class="form-group">
                            <label class="control-label"><?php echo $entry_carrier; ?></label>
                            <select name="carrier[<?php echo $order['order_id']; ?>]"
                                    class="form-control amazonus_carrier"
                                    id="amazonus_carrier_<?php echo $order['order_id']; ?>">
                                <?php foreach($market_options['amazonus']['carriers'] as $courier) { ?>
                                <?php echo '<option'.($courier == $market_options['amazonus']['default_carrier'] ? ' selected' : '').'>
                                '.$courier.'</option>'; ?>
                                <?php } ?>
                                <option value="other"><?php echo $text_other; ?></option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="carrier_other[<?php echo $order['order_id']; ?>]" value=""
                                   placeholder="<?php echo $entry_other; ?>" class="form-control"
                                   id="amazonus_carrier_<?php echo $order['order_id']; ?>_other"/>
                        </div>
                        <div class="form-group">
                            <label class="control-label"><?php echo $entry_tracking_no; ?></label>
                            <input type="text" name="tracking[<?php echo $order['order_id']; ?>]" value=""
                    ...
                    <td class="text-left"><textarea name="comments[<?php echo $order['order_id']; ?>]"
                                                    class="form-control" rows="3"></textarea></td>
                    <td class="text-center"><input type="hidden" name="notify[<?php echo $order['order_id']; ?>]"
                                                   value="0"/>
                        <input type="checkbox" name="notify[<?php echo $order['order_id']; ?>]" class="notify_checkbox"
                               value="1"/></td>
                </tr>
                <?php } ?>
                </tbody>
