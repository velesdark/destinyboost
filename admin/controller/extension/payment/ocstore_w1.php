...
class ControllerExtensionPaymentOcstoreW1 extends Controller
{
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->request->post['mer_id'] = trim($this->request->post['mer_id']);
        ...
        if (isset($this->request->post['mer_id'])) {
            $data['mer_id'] = $this->request->post['mer_id'];
        } else {
            $data['mer_id'] = $this->config->get('mer_id');
        }
